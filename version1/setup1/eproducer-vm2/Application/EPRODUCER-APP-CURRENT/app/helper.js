...
const getRegisteredUser = async (username, userOrg, isJson) => {
  ...
  let secret;
  try {
    if (username == "auto-conversion-handler") {
      // Register the user, enroll the user, and import the new identity into the wallet.
      secret = await ca.register(
        {
          affiliation: "eproducer.conversionhandler",
          enrollmentID: username,
          role: "client",
          attrs: [{ trustedOperator: true, ecert: true }],
        },
        adminUser
      );
    } else {
  ...
  return [response, enrollment.certificate];
};
