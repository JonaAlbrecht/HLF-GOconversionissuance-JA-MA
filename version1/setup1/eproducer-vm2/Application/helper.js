...
const getCCP = async () => {
  const ccpPath = path.resolve(__dirname, "connection-profile-SmartMeter.json");
...
const getCaUrl = async (ccp) => {
  const caURL = ccp.certificateAuthorities["ca.issuer.GOnetwork.com"].url;
  return caURL;
};

const getWalletPath = async () => {
  const walletPath = path.join(process.cwd(), "SmartMeter-wallet");
  return walletPath;
};

const getRegisteredUser = async (username, isJson) => {
