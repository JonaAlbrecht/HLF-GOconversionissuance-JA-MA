...
const query = async (channelName, chaincodeName, args, fcn, username, org_name) => {
    ...
    try {
        ...
        if (fcn == "queryGO" || fcn =="queryGOsByOwner" || fcn == 'getHistoryForAsset' || fcn=='restictedMethod') {
            console.log(`arguments type is------------------------------------------------------------- ${typeof args}`)
            console.log(`length of args is------------------------------------------------------------ ${args.length}`)
            result = await contract.evaluateTransaction(fcn, args[0]);

        } else if (fcn == "readPrivateGO" || fcn == "queryPrivateDataHash"
        || fcn == "collectionGOPrivateDetails") {
