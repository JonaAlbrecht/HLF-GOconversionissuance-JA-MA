...
# enroll the ca for buyer org cryptomaterial creation, the flag --tls.certfiles paths to the issuing body certificate authority root certificate
  ...
  Enable: true
  ClientOUIdentifier:
    Certificate: cacerts/localhost-10054-ca-eproducer-GOnetwork-com.pem
...
# this creates the Smart Meter x509 certificate with the attributes "trusted device = true, maxefficiency = 50 kW and technologyType = solar"
  fabric-ca-client register --caname ca.issuer.GOnetwork.com --id.name SmartMeter1 --id.secret Meter1pw --id.type client --id.attrs 'trustedDevice=true:ecert,maxEfficiency=50:ecert, technologyType=solar:ecert' --tls.certfiles ${PWD}/../../issuer-vm3/create-cryptomaterial-issuer/fabric-ca/issuer/tls-cert.pem
  ...
  cp ${PWD}/../crypto-config/peerOrganizations/eproducer.GOnetwork.com/peers/e-peer0.eproducer.GOnetwork.com/msp/cacerts/* ${PWD}/../crypto-config/peerOrganizations/eproducer.GOnetwork.com/ca/ca.eproducer.GOnetwork.com-cert.pem
