...
const getCCP = async (org) => {
    ...
    if (org == "buyer") {
        ...
        ccpPath = path.resolve(__dirname, '..', 'config', 'connection-eproducer.json');
    } else
        return null
...
const getCaUrl = async (org, ccp) => {
    let caURL;
    if (org == "buyer") {
        caURL = ccp.certificateAuthorities['ca.buyer.GOnetwork.com'].url;

    } else if (org == "eproducer") {
        caURL = ccp.certificateAuthorities['ca.eproducer.GOnetwork.com'].url;
    } else
        return null
    return caURL
...
const getRegisteredUser = async (username, userOrg, isJson) => {
    ...
    try {
        if (username == "superuser") {
            ...
            secret = await ca.register({ affiliation: 'buyer.department1', enrollmentID: username, role: 'client', attrs: [{ name: 'role', value: 'admin', ecert: true }] }, adminUser);
...
const getCaInfo = async (org, ccp) => {
    ...
    if (org == "buyer") {
        ...
        caInfo = ccp.certificateAuthorities['ca.eproducer.GOnetwork.com'];
    } else
        return null
    return caInfo
